...
def plot_poisson_pmf(lambda_param=8, max_l=20):
    ...
    #参数：lambda_param (float): 泊松分布参数λ， max_l (int): 最大的l值
...
def simulate_coin_flips(n_experiments=10000, n_flips=100, p_head=0.08):
   ...
   # 参数:n_experiments (int): 实验组数N，n_flips (int): 每组抛硬币次数，p_head (float): 正面朝上的概率
        
   # 返回: ndarray: 每组实验中正面朝上的次数
...
def compare_simulation_theory(n_experiments=10000, lambda_param=8):
    ...
    #参数:n_experiments (int): 实验组数，lambda_param (float): 泊松分布参数λ
